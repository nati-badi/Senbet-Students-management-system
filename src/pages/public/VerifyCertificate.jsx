...
export default function VerifyCertificate() {
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      setDownloading(true);
      const hide = message.loading('Generating Official Document...', 0);
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] flex flex-col items-center" ref={containerRef}>
      ...
      {/* 📥 FLOATING DOWNLOAD BUTTON (EXTREMELY VISIBLE) */}
      <button 
      ...
      <div className="flex-1 w-full flex flex-col items-center pt-8 pb-20 px-2 overflow-x-hidden">
        
        {/* CERTIFICATE SCALER */}
        <div className="cert-scaler" style={{ 
          transform: `scale(${scale})`, 
          transformOrigin: 'top center',
          marginBottom: `calc(297mm * ${1 - scale} * -1)`
        }}>
          <div ref={certRef} className="cert-body" style={{
            ...
            <div style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', opacity: 0.02, zIndex: 1, pointerEvents: 'none' }}>
            ...
            {/* Student Info */}
            <div className="cert-card" style={{ padding: '30px 40px', marginBottom: '24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', zIndex: 10 }}>
              <div>
                <span className="cert-amharic" style={{ fontSize: '11px', color: '#666', fontWeight: '800' }}>ሙሉ ስም / FULL NAME</span>
                <span className="cert-amharic" style={{ fontSize: '42px', fontWeight: '950', display: 'block', lineHeight: 1 }}>{student.name}</span>
                {!!(student?.baptismalName || student?.baptismalname) && (
                  <span className="cert-amharic" style={{ fontSize: '18px', fontWeight: '800', color: '#0F3A2B', display: 'block', marginTop: '6px' }}>{student.baptismalName || student.baptismalname}</span>
                )}
              </div>
            ...
            {/* Table */}
            <div className="cert-card" style={{ flex: 1, padding: '24px 32px', zIndex: 10 }}>
            ...
            <div style={{ marginTop: '30px', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', padding: '0 20px', zIndex: 10 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '14px' }}>
                ...
                <div style={{ maxWidth: '140px' }}>
                  ...
                  <span style={{ fontSize: '9px', color: '#666' }}>Verified digital copy of the official transcript.</span>
                </div>
              </div>
              <div style={{ display: 'flex', gap: '48px' }}>
      ...
      <style dangerouslySetInnerHTML={{__html: `
        .cert-scaler { box-shadow: 0 30px 60px rgba(0,0,0,0.15); border-radius: 4px; overflow: hidden; background: white; transition: transform 0.2s ease; }
        .cert-card { background: white; border: 1px solid #E2E8F0; border-radius: 12px; }
        ...
        @media print { .hide-on-print { display: none !important; } .cert-scaler { transform: none !important; margin: 0 !important; box-shadow: none !important; } }
      `}} />
    </div>
  );
}
