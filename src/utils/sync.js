        ...
        for (const tableName of TABLES) {
            try {
                ...
                if (data && data.length > 0) {
                    ...
                    const unsyncedLocal = await tableDb.where('synced').equals(0).toArray();
                    const unsyncedIds = new Set(unsyncedLocal.map(r => r.id));

                    const localReadyData = data
                        .filter(record => !unsyncedIds.has(record.id)) // PROTECT: Skip if local has unsynced changes
                        .map(record => {
                    ...
                    if (localReadyData.length > 0) {
                        ...
                        pulledCount += localReadyData.length;
                        console.log(`Pulled ${localReadyData.length} records into ${tableName}`);
                    }
                }
            } catch (err) {
                console.warn(`Critical error pulling table ${tableName}:`, err);
            }
        }
